...
class Yaoi:
    ...
    def _get_tag(self):
        if self._path:
            self._list_of_pages.add(self._path)
            return
    ...
    def _get_link(self, ch, ch_name):
        ...
        for full_tag in self._list_of_pages:
            if self._path:
                soup = get_file_soup(full_tag)
            else:
                soup = get_soup(full_tag)
            images = soup.find_all('img')
            for image in images:
                pgs = image.get('src')
                if self._path and pgs.startswith('https'):
                    continue
                if pgs.endswith('.gif'):
                    continue
                if 'bp.blogspot.com' in pgs:
                    continue
                if 'svg' in pgs:
                    continue
    ...
    def _get_user_args(self):
        ...
        parser.add_argument("--mf", help="Download this manga form the saved page", type=str)
    ...
    --m https://myreadingmanga.info/seki-sabato-tsukuru-matsuri-no-mae-kr/
        ...
        if not self._path.exists():
            print('File (Saved manga page) is not exists!')
            exit(0)
